...
class InstagramGraphQLClient:
    """
    Instagram GraphQL client that replicates the working instagram-video-downloader 
    ...
    def generate_request_body(self, shortcode: str) -> str:
        """Generate request body EXACTLY like the working TypeScript implementation"""
        
        # Use EXACT values and order from working implementation
        body_data = {
            "av": "0",
            "__d": "www",
            "__user": "0",
            ...
            "server_timestamps": True,  # Keep as boolean like their implementation
            "doc_id": self.doc_id,
        }
        
        # Use urlencode to match their querystring.stringify behavior
        return urlencode(body_data)
    ...
    async def get_video_info(self, url: str) -> Dict[str, Any]:
        """
        Get video information from Instagram URL using EXACT working approach
        NO session initialization - just direct GraphQL request like their implementation
        """
        ...
        # SIMPLE APPROACH: Direct GraphQL request like instagram-video-downloader
        graphql_url = "https://www.instagram.com/graphql/query"
        headers = self.get_headers(shortcode)
        body = self.generate_request_body(shortcode)
        
        # Add referrer header like their implementation
        headers['Referer'] = f"https://www.instagram.com/p/{shortcode}/"
        
        try:
            print(f"[INFO] Making direct GraphQL request (no session init)...")
            print(f"[DEBUG] Request URL: {graphql_url}")
            print(f"[DEBUG] Body length: {len(body)}")
            print(f"[DEBUG] Headers count: {len(headers)}")
            
            # Make direct POST request without session complexity
            # CRITICAL: Don't use session - use direct requests.post like their fetch()
            response = requests.post(
                graphql_url,
                headers=headers,
                data=body,
                timeout=30,
                # CRITICAL: Allow cookies but don't manage complex session
            ...
            if response.status_code == 200:
                try:
                    data = response.json()
                    print(f"[DEBUG] JSON keys: {list(data.keys()) if isinstance(data, dict) else 'Not a dict'}")
                except json.JSONDecodeError as je:
                    print(f"[ERROR] JSON decode error: {je}")
                    raise InstagramGraphQLError(f"Invalid JSON response: {response.text[:200]}")
                
                # Check response structure exactly like their implementation
                if not data.get('data'):
                    raise InstagramGraphQLError(f"Missing data key: {json.dumps(data)}")
                    
                if not data['data'].get('xdt_shortcode_media'):
                    raise InstagramGraphQLError(f"Missing media data: {json.dumps(data['data'])}")
                
                media = data['data']['xdt_shortcode_media']
                
                if not media:
                    raise InstagramGraphQLError(f"Media is null: {json.dumps(data['data'])}")
                
                if not media.get('is_video', False):
                    raise InstagramGraphQLError(f"Not a video post: is_video={media.get('is_video')}")
                
                video_url = media.get('video_url')
                if not video_url:
                    raise InstagramGraphQLError(f"No video URL found. Keys: {list(media.keys())}")
                ...
                print(f"[SUCCESS] Video info extracted for {shortcode}")
                return result
                
            else:
                error_text = response.text
                raise InstagramGraphQLError(f"HTTP {response.status_code}: {error_text}")
    ...
    def download_video_direct(self, video_url: str, output_path: str) -> bool:
        """Download video directly from Instagram's CDN"""
        try:
            headers = {
                "User-Agent": "Mozilla/5.0 (Linux; Android 11; SAMSUNG SM-G973U) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/14.2 Chrome/87.0.4280.141 Mobile Safari/537.36",
